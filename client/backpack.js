...
const sold = async (ticket) => {
    await contract.methods.wannasold(ticket.id).send({from: account});
    await refreshyours();
};


const regret = async (ticket) => {
    item = await contract.methods.items(ticket.id).send({ from: account });
...
const refreshyours = async () => {
    ticketsEl.innerHTML = '';
    for (let i = 0; i < TOTAL_ITEMS; i++) {
        const ticket = await contract.methods.items(i).call();
        ticket.id = i;
        if (ticket.owner == account && ticket.status == false) {
                    ...
                    <div class="card-body">
                        <p class="card-text">${ticket.price / 1e18} &nbsp Maple Coin</p>
                            <div class="d-grid gap-3 col-6 mx-auto">
                                <button id = "buy_button" class="btn btn-danger btn-lg">Sell</button>
                            </div>
                    </div>
                </div>`
            );
            ticketEl.onclick = sold.bind(null, ticket);
...
const refreshsell = async () => {
    ticketsEl.innerHTML = '';
    for (let i = 0; i < TOTAL_ITEMS; i++) {
        const ticket = await contract.methods.items(i).call();
        ticket.id = i;
        if (ticket.owner == account && ticket.status == true) {
            const ticketEl = createElementFromString(
                `<div class="ticket card" style="width: 20rem;">
                    <img src="${data[i]}" class="card-img-top" alt="...">
                    <div class="card-body">
                        <p class="card-text">${ticket.price / 1e18} &nbsp Maple Coin</p>
                            <div class="d-grid gap-3 col-6 mx-auto">
                                <button id = "buy_button" class="btn btn-danger btn-lg">Regret</button>
                            </div>
                    </div>
                </div>`
            );
            ticketEl.onclick = regret.bind(null, ticket);
            ticketsEl.appendChild(ticketEl);
        }
    }
};
